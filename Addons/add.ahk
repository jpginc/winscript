...
{	while(true)
	{	;get the name of the new code segment
		...
		{	
			...
			{	
				fileLocation := A_ScriptDir "\Addons\" newShortcut ".ahk"
			} else if(FileExist(A_scriptdir "\Addons\" newShortcut "\" newShortcut ".ahk"))
			{
				fileLocation := A_ScriptDir "\Addons\" newShortcut "\" newShortcut ".ahk"
			} else
			{	
				...
				fileLocation := RegExReplace(dir, "\R")
				controller.clearDisplay()
			}
			
			codeReader := new JPGIncCodeReader(fileLocation)
			newCode := codeReader.readCode()
			if(! newCode)
			{	
