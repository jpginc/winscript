...
JPGIncShortcuts := "add,update,remove,edit,"
...
{	__new(controller)
	{	while(true)
		{	toEdit := controller.getChoice(controller.getAllShortcuts(), "Select a code segment/shortcut to remove")
			if(toEdit == "cancelled")
			{	return
			}
			if(controller.codeOrShortcutExists(newShortcut))
			{	MsgBox, , Error, Error that shortcut is invalid or already in use
			} else
			{	IfExist, % A_scriptdir "\Addons\" newShortcut ".ahk"
				{	FileRead, newCode, % A_ScriptDir "\Addons\" newShortcut ".ahk"
